...
print("Num anchors: %d" %len(ANCHORS))

train_tfrecord   = "./VOC_tfrecord/VOC_train.tfrecords"
test_tfrecord    = "./VOC_tfrecord/VOC_test.tfrecords"
...
checkpoint_path = './checkpoint/yolo-v3-416/model.ckpt'
...
writer_train = tf.summary.FileWriter("./logs/voc-416/train")
writer_test  = tf.summary.FileWriter("./logs/voc-416/test")
...
for step in range(STEPS):
    ...
    if (step+1) % SAVE_INTERNAL == 0:
        saver.save(sess, save_path=checkpoint_path, global_step=step+1)

    sys.stdout.flush()
    sys.stdout.write("=> STEP %10d [TRAIN]:\tloss_xy:%7.4f \tloss_wh:%7.4f \tloss_conf:%7.4f \tloss_class:%7.4f\r"
